...
def run_e2e_demand_to_supply_bridge_flow_smoke(
    *,
    ...
    result = E2EDemandToSupplyBridgeFlowSmokeResult()

    source_node = next((n for n in iter_nodes(outbound_root) if getattr(n, "name", None) == source_node_name), None)
    source_lots = set()
    ...
    mom_allocated_lots = 0
    for node in iter_nodes(inbound_root):
        if str(getattr(node, "name", "")).startswith("MOM"):
            psi = getattr(node, "psi4demand", []) or []
            for week in psi:
                if isinstance(week, list) and len(week) > PSI_BUCKET_INDEX["S"] and isinstance(week[PSI_BUCKET_INDEX["S"]], list):
                    mom_allocated_lots += len(week[PSI_BUCKET_INDEX["S"]])
    result.mom_allocated_lot_count = mom_allocated_lots

    cap_result = capacity_aware_inbound_backward_planning(
    ...
    result.capacity_planned_lot_count = cap_result.accepted_lot_count
    result.shifted_lot_count = cap_result.shifted_lot_count
    result.backlog_lot_count = cap_result.backlog_lot_count
    ...
    backlog_lots = {entry.get("lot_id") for entry in cap_result.backlog_lots if isinstance(entry.get("lot_id"), str)}
    final_lots = demand_lots | supply_lots | backlog_lots
    ...
    result.non_list_bucket_errors = d_non_list + s_non_list + cap_result.non_list_bucket_errors + bridge_b_result.non_list_bucket_errors
    result.non_string_lot_errors = d_non_string + s_non_string + cap_result.non_string_lot_errors

    result.message = "Completed Bridge A -> MOM allocation -> capacity-aware inbound backward planning -> Bridge B smoke flow."
    return result
