...
class RealLikeNode:
    """Real-WOM-compatible minimal adapter for node PSI tests."""

    def __init__(self, name4node: str):
        self.name4node = name4node
        self.children = []
        self.psi4demand = {}
        self.psi4supply = {}
...
def test_with_capacity_forward_planning_real_node_p_cap():
    week = "2026-W01"
    node = RealLikeNode("REAL_LIKE_MOM")
    node.init_week(week)
...
def test_with_capacity_forward_planning_real_node_s_cap():
    week = "2026-W01"
    node = RealLikeNode("REAL_LIKE_DAD")
    node.init_week(week)
...
def test_with_capacity_forward_planning_real_node_i_cap_hard():
    week = "2026-W01"
    node = RealLikeNode("REAL_LIKE_COLD_DC")
    node.init_week(week)
    node.psi4supply[week][2] = ["I1", "I2", "I3", "I4"]
